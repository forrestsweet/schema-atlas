...
export type ThreadComponents = {
  ...
  ComposerModelSelector?: ReactNode;
...
const canShowWelcomeSuggestions = (s: AssistantState) =>
  isNewChatView(s) &&
  s.composer.isEmpty &&
  !s.threads.isLoading &&
  !s.thread.isLoading &&
  !s.thread.isDisabled &&
  s.threadListItem.status !== "new";
      ...
      >
        ...
        >
          ...
          >
            ...
            <Composer />
            <AuiIf condition={canShowWelcomeSuggestions}>
              <ThreadSuggestions />
            </AuiIf>
          </ThreadPrimitive.ViewportFooter>
        </div>
...
const ThreadSuggestions: FC = () => {
  return (
    <div className="aui-thread-welcome-suggestions flex w-full flex-col gap-1 px-2">
  ...
  return (
    <div className="aui-thread-welcome-suggestion-display">
      <SuggestionPrimitive.Trigger send asChild>
        <Button
          variant="ghost"
          className="aui-thread-welcome-suggestion h-auto min-h-11 w-full justify-start gap-3 px-3 py-2 text-left font-normal whitespace-normal"
        >
          <div className="min-w-0 flex-1">
            <SuggestionPrimitive.Title className="aui-thread-welcome-suggestion-text-1 block truncate text-sm font-medium" />
            <SuggestionPrimitive.Description className="aui-thread-welcome-suggestion-text-2 block truncate text-xs leading-4 text-muted-foreground empty:hidden" />
          </div>
...
const Composer: FC = () => {
  const isPreparing = useAuiState(
    (state) =>
      state.threads.isLoading ||
      state.thread.isLoading ||
      state.threadListItem.status === "new",
  );

  if (isPreparing) {
    return <Skeleton className="h-[86px] w-full rounded-(--composer-radius)" />;
  }
...
const ComposerAction: FC = () => {
  const { ComposerModelSelector } = useContext(ThreadComponentsContext);
